...
def smo_basic_loop(pop_size=20, max_iter=50, wait_iterations=10, max_r=50, path_size=4, scenario=None, n_neighbours=5):
    ...
    for iter in range(max_iter):
        ...
        if strategy == "exploitation":
            ...
            new_individuals, new_fits, new_cardinality, path, path_fits = exploitation(X=selected,n=n_neighbours,max_r=max_r,iter=iter,fits=best_fitness,C_X=10, scenario=scenario,all_fits=best_fitness_history)

        else:
            X_best, fit_best, path, path_fits = exploration(x=selected, max_R=500, p_end=0.3, p_back=0.1,scenario=scenario)
